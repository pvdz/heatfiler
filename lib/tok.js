(function(exports){
  ...
  var proto = {
    ...
    /** Add line information to tokens. The last token
     * on a line of code is a carriage return (⏎). Single
     * line or multi-line comments are retrieved as a single
     * comment token.
     * */
    addLineCount: function(token, type, lastValue){
      token['line'] = this.lineCounter;
      if(type !== WHITE){
        // This token is anything other than a line terminator
        // and a single or multiple comment line.
        return;
      }

      var carriageMatches = lastValue.match(/[\r\n]/g);
      if(carriageMatches) {
          this.lineCounter += carriageMatches.length;
      }
    },
