(function(exports){
  ...
  var proto = {
    ...
    lastToken: null,
    /** Counts number of lines for use in tokens */
    lineCounter: 1,
    ...
    next: function(expressionStart){
      ...
      do {
        ...
        if (toStream) {
          ...
          var token = {type:type, value:lastValue, start:this.lastStart, stop:this.pos, white:this.tokens.length};
          this.addLineCount(token, type, lastValue);
    ...
    /** Add line information to tokens. */
    addLineCount: function(token, type, lastValue){
      if(type === WHITE){
        this.updateLineCounterForWhiteTokens(lastValue);
      }
      else{
          var carriageMatches = lastValue.match(/[\r\n]/g);
          if(!carriageMatches){
    ...
    /** Increment line counters when line terminators are encountered */
    updateLineCounterForWhiteTokens: function(lastValue){
      if(lastValue.length > 1){
        // Single and multi-line comments are not of interest to us
        // and are thus neglected.
        var carriageMatches = lastValue.match(/[\r\n]/g);
        if(carriageMatches) {
            this.lineCounter += carriageMatches.length + 1;
        }
      }
